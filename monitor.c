...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/15 20:22:08 by brian             #+#    #+#             */
/*   Updated: 2025/01/18 18:43:14 by brian            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	is_all_philos_active(t_mtx *mutex, long *threads, long ph_total)
{
	bool	value;
...
static bool	is_philo_dead(t_ph *philo)
{
	long	elapsed;
	long	dying_time;
	...
	if (get_bool(&philo->ph_mutex, &philo->max_meals))
		return (false);
	elapsed = get_time(MILLISECONDS) - get_long(&philo->ph_mutex,
			&philo->meal_time);
...
void	*death_affirm(void *ph_data)
{
	...
	while (!get_bool(&data->access_mutex, &data->end_time))
	{
		...
		while (i < data->ph_total && !get_bool(&data->access_mutex,
				&data->end_time))
		{
			if (is_philo_dead(data->philos_arr + i))
			{
